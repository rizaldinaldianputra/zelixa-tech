...
export default function ContactSection({ theme = 'dark', lang }: SectionProps) {
  ...
  return (
    <section id="contact" className={`py-24 px-6 border-t relative overflow-hidden transition-colors duration-500 ${
      ...
      <div className="max-w-[1200px] mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
          ...
          <div className="lg:col-span-7">
            <div className={`border rounded-3xl p-8 md:p-10 shadow-lg relative overflow-hidden ${
              isDark ? 'bg-slate-900/40 border-slate-900' : 'bg-slate-50 border-slate-200'
            }`}>
              ...
              {isSuccess ? (
                <div className="text-center py-12 space-y-6 animate-fade-in-up">
                  ...
                  <button 
                    ...
                    className={`px-6 py-2.5 rounded-xl border text-xs transition-colors cursor-pointer ${
                      isDark 
                ...
                <form onSubmit={handleSubmit} className="space-y-6 text-left">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <input 
                        ...
                        className={`w-full px-5 py-3.5 border rounded-xl text-sm transition-all focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 ${
                          isDark 
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      {budgetOptions.map((opt, oIdx) => (
                        <button
                          ...
                          className={`py-3 rounded-xl border text-xs font-semibold transition-colors cursor-pointer ${
                            formData.budget === opt
                              ? isDark 
                                ? 'bg-blue-600/10 border-blue-500/50 text-blue-450' 
                                : 'bg-blue-50 border-blue-400 text-blue-600'
                              : isDark 
                                ? 'bg-slate-950 border-slate-800 text-slate-400 hover:border-slate-700 hover:text-slate-350'
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea 
                      ...
                      className={`w-full px-5 py-4 border rounded-xl text-sm transition-all resize-none focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 ${
                        isDark 
                          ? 'bg-slate-950 border-slate-800 text-white placeholder-slate-600' 
                          : 'bg-white border-slate-200 text-slate-900 placeholder-slate-400'
                      }`}
          ...
          <div className="lg:col-span-5 space-y-8 text-left">
            <div className={`border rounded-3xl p-8 space-y-8 ${
              isDark ? 'bg-slate-900/20 border-slate-900' : 'bg-slate-50 border-slate-200'
            }`}>
              ...
              <div className={`relative aspect-[16/10] rounded-2xl overflow-hidden border p-4 flex flex-col justify-end text-left select-none bg-gradient-to-tr group ${
                isDark ? 'bg-slate-950 border-slate-850 from-slate-950 to-slate-900' : 'bg-slate-100 border-slate-200 from-slate-100 to-slate-50'
              }`}>
